...
def hack_archive(file_name):
    ...
    while True:
        ...
        """
        pass1 = build_pass()
        if pass1 not in wrong_passwords:
            if extract_archive(file_to_open, pass1):
                break
            else:
                wrong_passwords.append(pass1)
                tries += 1
                if tries == 10**PASSWORD_LENGTH:
                    print(f'Incorrect password. It must consist of {PASSWORD_LENGTH} digits.')
                    return 'Done!'

    print(f'Archive {file_name} is hacked. Password - {pass1}')
